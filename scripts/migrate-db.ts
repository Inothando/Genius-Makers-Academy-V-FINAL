...
async function run() {
  console.log('====================================================');
  console.log('🏁 CLOUD DOCUMENT METADATA ALIGNMENT ENGINE');
  console.log('====================================================');
  ...
  console.log('📬 STEP 1: Retrieving all cloud past papers from Firestore...');
  const snap = await getDocs(collection(db, 'past-papers'));
  const totalDocs = snap.size;
  console.log(`✅ Retrieved ${totalDocs} documents from the cloud.`);

  console.log('\n⚙️ STEP 2: Running alignments and grouping batched changes...');
  const updatesToApply: { id: string; docObj: any }[] = [];
  ...
  snap.forEach(docSnap => {
    ...
    const fileMeta = parseStorageFile(data.storagePath, data.storagePath.split('/').pop() || '');

    // Check if anything has mismatched or if province/session is outdated/missing
    const isMismatched = 
      data.title !== fileMeta.title ||
      data.subject !== fileMeta.subject ||
      data.grade !== fileMeta.grade ||
      data.year !== fileMeta.year ||
      data.curriculum !== fileMeta.curriculum ||
      data.paperNumber !== fileMeta.paperNumber ||
      data.type !== fileMeta.type ||
      data.language !== fileMeta.language ||
      data.session !== fileMeta.session ||
      data.province !== fileMeta.province;
    ...
    if (isMismatched) {
      updatesToApply.push({
        ...
        docObj: {
          ...
          year: fileMeta.year,
          curriculum: fileMeta.curriculum,
          paperNumber: fileMeta.paperNumber,
          type: fileMeta.type,
          language: fileMeta.language,
          session: fileMeta.session,
          province: fileMeta.province,
        }
      });
    }
  });

  console.log(`📊 Found ${updatesToApply.length} out of ${totalDocs} documents containing outdated or mismatched metadata!`);

  if (updatesToApply.length === 0) {
    console.log('💯 Every single document has perfectly synchronized metadata already!');
    return;
  }

  console.log(`\n📦 STEP 3: Committing updates to Cloud Firestore concurrently in high-efficiency batches of 400...`);
  ...
  for (let i = 0; i < updatesToApply.length; i += BATCH_SIZE) {
    ...
    chunk.forEach(update => {
      const ref = doc(db, 'past-papers', update.id);
      batch.update(ref, update.docObj);
    });

    const promise = batch.commit().then(() => {
      successCount += chunk.length;
      console.log(`   ✨ Batch of ${chunk.length} modifications committed successfully!`);
    }).catch(async (batchErr: any) => {
      console.log(`   ⚠️ Batch failed with error: ${batchErr.message}. Retrying document by document for this chunk...`);
      for (const update of chunk) {
        const ref = doc(db, 'past-papers', update.id);
        try {
          const singleBatch = writeBatch(db);
          singleBatch.update(ref, update.docObj);
          await singleBatch.commit();
          successCount++;
        } catch (singleErr: any) {
          console.error(`     ❌ Failed to write document ${update.id} (${update.docObj.title}): ${singleErr.message}`);
        }
      }
    });
  ...
  console.log(`🚀 Executing all ${batchPromises.length} parallel database write batches...`);
  await Promise.all(batchPromises);

  console.log(`\n🎉 Success! Synchronized all ${successCount} Cloud records.`);
}
