...
import { initializeFirestore, collection, getDocs, writeBatch, doc, serverTimestamp } from 'firebase/firestore';
...
async function run() {
  console.log('====================================================');
  console.log('🏁 TERMINAL STORAGE & DB RECONCILIATION ENGINE');
  console.log('====================================================');
  ...
  console.log(`🤖 Target Database: ${config.firestoreDatabaseId || '(default)'}`);
  console.log(`🪣 Target Store Bucket: ${config.storageBucket}`);
  ...
  console.log('\n🔍 STEP 1: Fetching current indexed database past papers...');
  const dbPathSet = new Set<string>();
  try {
    const querySnapshot = await getDocs(collection(db, 'past-papers'));
    querySnapshot.forEach(doc => {
      const data = doc.data();
      if (data.storagePath) {
        dbPathSet.add(data.storagePath);
      }
    });
    console.log(`✅ Successfully queried Firestore. Found ${dbPathSet.size} currently indexed papers.`);
  } catch (err: any) {
    console.error('❌ Failed to connect to Firestore:', err.message);
    process.exit(1);
  }
  
  console.log('\n📂 STEP 2: Scanning physical folders recursively inside Firebase Storage...');
    ...
    try {
      ...
      for (const item of result.items) {
        ...
        if (discoveredFiles.length % 50 === 0) {
          console.log(` - Crawled ${discoveredFiles.length} files...`);
        }
      }
      ...
      console.warn(` ⚠️ Crawl warning on path '${pathStr}': ${err.message}`);
    }
  };
  
  // Folders to target
  const foldersToCrawl = ['Past-papers', 'past-papers', 'Past-paper', 'past-paper', 'Past-Papers'];
  for (const folderName of foldersToCrawl) {
    console.log(`⚡ Checking folder: "${folderName}"...`);
    try {
      const folderRef = ref(storage, folderName);
      await crawl(folderRef);
    } catch (err: any) {
      console.log(` - Folder "${folderName}" not found or not traversable.`);
    }
  }
  
  console.log(`\n🎉 Scan Finished. Discovered ${discoveredFiles.length} physical file assets in Firebase Storage.`);
  
  const missingFiles = discoveredFiles.filter(item => !dbPathSet.has(item.fullPath));
  console.log(`📊 RECONCILIATION SUMMARY:`);
  console.log(` - Already indexed in DB: ${discoveredFiles.length - missingFiles.length}`);
  console.log(` - Missing from DB (requires import): ${missingFiles.length}`);
  
  if (missingFiles.length === 0) {
    console.log('\n✅ System is completely in sync! No files to write.');
    process.exit(0);
  }
  
  console.log(`\n📝 STEP 3: Indexing ${missingFiles.length} past papers into Firestore in high-efficiency batches...`);
  ...
  for (let i = 0; i < missingFiles.length; i += BATCH_SIZE) {
    ...
    console.log(`🚀 Indexing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(missingFiles.length / BATCH_SIZE)} (${chunk.length} papers)...`);
    
    await Promise.all(chunk.map(async (fileRef) => {
      try {
        ...
        console.error(` ⚠️ Failed parsing file URL/meta for ${fileRef.name}:`, e.message);
      }
    }));
    
    if (batchHasDocs) {
      await batch.commit();
      console.log(` ✨ Successfully committed index batch to Firestore. Cumulative total indexed: ${importedCount}/${missingFiles.length}`);
    }
    
    // Slight pause to let things cool down and ensure smooth execution
    await new Promise(r => setTimeout(r, 100));
  }
  
  console.log('\n====================================================');
  console.log(`🌟 CRADLE SUCCESS! Indexed ${importedCount} items into Firestore.`);
  console.log('Your website list is now fully loaded on the live site! ✨');
  console.log('====================================================');
}
