...
export function DiscussionPostCard({ post }: DiscussionPostCardProps) {
  ...
  return (
    <motion.div 
      ...
      className="bg-white rounded-[2rem] sm:rounded-[3rem] border border-border-subtle overflow-hidden hover:border-primary/20 transition-all duration-300"
    >
      <div className="p-6">
        {/* Author Header */}
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary group-hover:bg-primary group-hover:text-lux-text transition-all">
              {post.authorName?.[0] || 'G'}
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                {post.isGuest && (
                  <span className="px-1.5 py-0.5 bg-surface text-[8px] font-black uppercase tracking-widest text-lux-text rounded-md border border-border-subtle">
                    Guest
                  </span>
                )}
              </div>
              <div className="flex items-center text-xs text-lux-text">
                <Clock size={12} className="mr-1" />
                {timeAgo(post.createdAt)}
              </div>
            </div>
          </div>
          <button className="p-2 hover:bg-surface rounded-full transition-colors text-lux-text">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <span className="px-2.5 py-1 bg-surface text-[10px] font-bold text-lux-text rounded-lg border border-border-subtle">
        ...
        <p className="text-lux-text leading-relaxed mb-6 whitespace-pre-wrap">
          {post.content}
        </p>

        {post.imageUrl && (
          <div className="mb-6 rounded-2xl sm:rounded-3xl overflow-hidden border border-border-subtle bg-surface">
          ...
          <div className="flex items-center gap-6">
            <button 
              ...
              className={cn(
                ...
                isLiked ? "text-red-500" : "text-lux-text hover:text-red-500"
              )}
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 text-sm font-bold text-lux-text hover:text-primary transition-all"
            >
              <MessageCircle size={20} />
              {post.replyCount}
            </button>
            <button className="flex items-center gap-2 text-sm font-bold text-lux-text hover:text-text-primary transition-all">
              <Share2 size={20} />
            </button>
          </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                     <span className="text-xs font-bold uppercase tracking-wider text-lux-text">Your Reply</span>
                  </div>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <span className="text-[10px] font-bold text-lux-text">Post as Guest</span>
                    <input 
                      type="checkbox" 
                ...
                <div className="relative">
                  <textarea 
                    ...
                    className="w-full bg-white border border-border-subtle rounded-2xl sm:rounded-3xl p-4 pr-14 text-sm outline-none focus:border-primary/30 transition-all resize-none shadow-sm"
                    value={newReply}
                  ...
                  <button 
                    ...
                    className="absolute right-3 bottom-3 p-2 bg-primary text-lux-text rounded-xl hover:bg-primary-dark transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
                  >
              ...
              <div className="space-y-6 pt-4">
                {replies.length === 0 ? (
                  <p className="text-center text-sm text-lux-text py-4 italic">No replies yet. Be the first to help!</p>
                ) : (
                  replies.map((reply) => (
                    <div key={reply.id} className="flex gap-4">
                      <div className="w-8 h-8 rounded-full bg-white border border-border-subtle flex items-center justify-center font-bold text-lux-text text-xs">
                        {reply.authorName?.[0] || 'S'}
                      </div>
                      <div className="flex-1">
                        <div className="bg-white p-4 rounded-2xl sm:rounded-3xl rounded-tl-none border border-border-subtle shadow-sm mb-1">
                          <div className="flex items-center justify-between mb-1">
                            <span className="font-bold text-xs text-text-primary">{reply.authorName}</span>
                            <span className="text-[10px] text-lux-text">{timeAgo(reply.createdAt)}</span>
                          </div>
