...
export function StudyPackVideoItem({ item, order, onWatch, metadata }: VideoItemProps) {
  return (
    <motion.div 
      initial={{ opacity: 0, x: -20 }}
      whileInView={{ opacity: 1, x: 0 }}
      viewport={{ once: true }}
      className="p-6 bg-white border border-border-subtle rounded-2xl flex items-center gap-6 group hover:shadow-lg transition-all"
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {metadata?.duration && <span className="text-[10px] font-bold text-text-tertiary">• {metadata.duration}</span>}
        </div>
...
export function StudyPackDocItem({ item, order, onPreview, metadata }: DocItemProps) {
  const getIcon = () => {
  ...
  return (
    <motion.div 
      ...
      className="p-6 bg-white border border-border-subtle rounded-2xl flex items-center gap-6 group hover:shadow-lg transition-all"
    >
      <div className="w-10 h-10 rounded-full bg-secondary/10 flex items-center justify-center shrink-0 font-bold text-secondary text-sm">
        {order}
      </div>

      <div className="w-12 h-12 bg-surface rounded-xl flex items-center justify-center text-text-tertiary shrink-0 border border-border-subtle group-hover:text-secondary group-hover:bg-secondary/5 transition-colors">
        {getIcon()}
      </div>
