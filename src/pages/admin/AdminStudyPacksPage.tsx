          ...
          <div className="bg-white border border-border-subtle rounded-[40px] shadow-lg p-10">
             <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                   <div className="w-12 h-12 bg-primary/10 rounded-2xl sm:rounded-3xl flex items-center justify-center text-primary">
                      <Plus size={24} />
                   </div>
                ...
                <div className="space-y-8">
                   <div className="grid grid-cols-2 gap-6">
                      <div className="col-span-2">
                         <label className="block text-xs font-black uppercase text-lux-text mb-2">Pack Title</label>
                         <input 
                           type="text" 
                           placeholder="e.g. Master Grade 12 Calculus" 
                           value={formData.title}
                           onChange={(e) => setFormData({...formData, title: e.target.value})}
                           className="w-full p-4 bg-surface border border-border-subtle rounded-2xl sm:rounded-3xl text-sm font-bold outline-none focus:border-primary/30"
                         />
                      </div>
                      <div className="col-span-2">
                         <label className="block text-xs font-black uppercase text-lux-text mb-2">Description</label>
                         <textarea 
                           placeholder="What will students learn in this pack?" 
                           value={formData.description}
                           onChange={(e) => setFormData({...formData, description: e.target.value})}
                           className="w-full h-32 p-4 bg-surface border border-border-subtle rounded-2xl sm:rounded-3xl text-sm outline-none focus:border-primary/30 resize-none"
                         />
                      </div>
                      <div>
                         <label className="block text-xs font-black uppercase text-lux-text mb-2">Subject</label>
                         <select 
                           value={formData.subject}
                           onChange={(e) => setFormData({...formData, subject: e.target.value})}
                           className="w-full p-4 bg-surface border border-border-subtle rounded-2xl sm:rounded-3xl text-sm font-bold outline-none"
                         >
                            {['MATHEMATICS', 'PHYSICAL SCIENCES', 'LIFE SCIENCES', 'ACCOUNTING'].map(s => <option key={s} value={s}>{s}</option>)}
                         </select>
                      </div>
                      <div>
                         <label className="block text-xs font-black uppercase text-lux-text mb-2">Grade</label>
                         <select 
                         ...
                         {sequence.length > 0 ? (
                           sequence.map((item, idx) => (
                             <div 
                               ...
                               className="p-4 bg-white border border-border-subtle rounded-2xl sm:rounded-3xl flex items-center gap-4 group cursor-move hover:border-primary/30 transition-all shadow-sm"
                             >
                                <GripVertical size={16} className="text-lux-text group-hover:text-primary transition-colors" />
                                <div className="w-6 h-6 rounded-full bg-surface border border-border-subtle flex items-center justify-center text-[10px] font-black text-lux-text">
                                   {idx + 1}
                                </div>
                                <div className={cn(
                                ...
                                <button 
                                  ...
                                  className="p-1.5 hover:bg-red-50 text-lux-text hover:text-red-500 rounded-lg transition-all"
                                >
                                   <X size={14} />
                                </button>
                             </div>
                           ...
                           <div className="py-20 border-2 border-dashed border-border-subtle rounded-[24px] flex flex-col items-center justify-center text-center px-10">
                              <Package size={32} className="text-lux-text mb-3 opacity-20" />
                              <p className="text-xs font-bold text-lux-text">Sequence is empty. Search and add content from the library.</p>
                           </div>
                         )}
                      </div>
                   </div>

                   <Button className="w-full py-4 text-base rounded-2xl sm:rounded-3xl shadow-xl shadow-primary/20" onClick={handleSave} disabled={isSaving}>
                ...
                <div className="space-y-8 h-full flex flex-col">
                   <div className="p-8 bg-surface rounded-[32px] border border-border-subtle">
                      ...
                      <form onSubmit={handleSearch} className="relative mb-6">
                         <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-lux-text" />
                         <input 
                           type="text" 
                           placeholder={`Search ${formData.subject.toLowerCase()} library...`} 
                           className="w-full pl-12 pr-4 py-3 bg-white border border-border-subtle rounded-2xl sm:rounded-3xl text-xs font-bold outline-none focus:border-primary/30"
                           value={searchQuery}
                      ...
                      <div className="space-y-8 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
                         ...
                         {searchResults.videos.length > 0 && (
                           <div>
                              <h5 className="text-[10px] font-black uppercase text-lux-text mb-3 flex items-center gap-2 tracking-widest">
          ...
          <div className="bg-white border border-border-subtle rounded-[40px] shadow-sm overflow-hidden">
             <div className="p-10 border-b border-border-subtle flex items-center justify-between">
                <div>
                   ...
                   <p className="text-xs text-lux-text font-bold uppercase tracking-widest">Structured learning sequences for scholars</p>
                </div>
                <Button onClick={() => setShowCreateForm(true)} className="rounded-2xl sm:rounded-3xl">
                ...
                <table className="w-full">
                   <thead>
                      <tr className="bg-surface text-left">
                         <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-lux-text">Study Pack</th>
                         <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-lux-text">Subject & Grade</th>
                         <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-lux-text text-center">Curriculum</th>
                         <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-lux-text text-center">Items</th>
                         <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-lux-text text-center">Enrolled</th>
                         <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-lux-text text-center">Visibility</th>
                             ...
                             <td className="px-8 py-5">
                                <div className="flex items-center gap-4">
                                   ...
                                   <div>
                                      ...
                                      <p className="text-[10px] font-medium text-lux-text">Created by {pack.creatorName}</p>
                                   </div>
                                </div>
                             </td>
                             <td className="px-8 py-5">
                             ...
                             <td className="px-8 py-5 text-center">
                                <button 
                                  ...
                                  className={cn(
                                    ...
                                    pack.isPublic ? "bg-green-50 text-green-600 border-green-100" : "bg-text-tertiary/10 text-lux-text border-border-subtle"
                                  )}
                                >
                             ...
                             <td className="px-8 py-5 text-right">
                                <div className="flex items-center justify-end gap-2">
                                   <Button variant="outline" size="sm" className="h-9 w-9 p-0 rounded-lg">
                                      <Edit size={16} className="text-lux-text" />
                                   </Button>
                                   <Button 
                                     variant="outline" 
                                     size="sm" 
